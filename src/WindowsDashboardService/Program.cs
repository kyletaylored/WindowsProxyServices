...
{
    ["OpenMeteo"]       = "/v1/forecast?latitude=40.71&longitude=-74.01&current_weather=true",
    ["JsonPlaceholder"] = "/todos/1",
    ["DatadogDemo"]     = "/api/lab/health",
...
app.MapGet("/api/services", async (IHttpClientFactory cf) =>
{
    ...
    {
        var svcName = inst.ServiceName ?? $"WindowsProxyService.{inst.InstanceName}";
        string status;
        try
        {
...
app.MapPost("/api/services/{name}/test", async (string name, TestRequest? req, IHttpClientFactory cf) =>
{
    ...
    // 30s to accommodate slow-query endpoints that can delay up to 8s
    ...
    {
        return Results.Ok(new { Url = url, Method = method, StatusCode = 0, Body = (object?)null, Error = ex.Message });
    }
});
...
{
    var inst    = instances.FirstOrDefault(i => string.Equals(i.InstanceName, name, StringComparison.OrdinalIgnoreCase));
    var svcName = inst?.ServiceName ?? $"WindowsProxyService.{name}";
    try
    {
        using var sc = new ServiceController(svcName);
